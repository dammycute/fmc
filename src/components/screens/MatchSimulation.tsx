...
const MatchSimulation: React.FC<MatchSimulationProps> = ({ match, onComplete }) => {
  ...
  return (
    <div className="fixed inset-0 bg-[#09090b] z-[100] flex flex-col overflow-hidden animate-in fade-in duration-500">
      ...
      {/* Main Container */}
      <div className="relative flex-1 flex flex-col max-w-5xl mx-auto w-full p-3 sm:p-6 gap-4 sm:gap-6 overflow-y-auto custom-scrollbar">
        {/* Match Header / Scoreboard */}
        <Card className="bg-zinc-900/50 border-white/5 shadow-2xl backdrop-blur-xl rounded-2xl sm:rounded-[2.5rem] overflow-hidden">
          <CardContent className="p-4 sm:p-8">
            <div className="flex items-center justify-between gap-3 sm:gap-8">
              <div className="flex-1 flex flex-col items-center gap-2 sm:gap-4">
                <div
                  className="w-14 h-14 sm:w-20 sm:h-20 rounded-2xl flex items-center justify-center text-2xl sm:text-4xl font-black text-white shadow-2xl border-2 sm:border-4"
                  style={{ backgroundColor: homeClub?.primaryColor, borderColor: 'rgba(255,255,255,0.1)' }}
                >
                  {homeClub?.name.charAt(0)}
                </div>
                <div className="text-center">
                  <h2 className="text-sm sm:text-xl font-black text-white tracking-tight uppercase italic leading-tight">{homeClub?.name}</h2>
                  <p className="text-[8px] sm:text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mt-0.5">HOME</p>
                </div>
              </div>

              <div className="flex flex-col items-center gap-2 sm:gap-4">
                <div className="px-3 sm:px-5 py-1.5 bg-white/5 border border-white/10 rounded-xl flex items-center gap-2">
                  <Timer className={cn("w-3 h-3 sm:w-4 sm:h-4", isFinished ? "text-zinc-500" : "text-indigo-400 animate-pulse")} />
                  <span className="text-sm sm:text-lg font-black font-mono text-white tracking-tighter">
                    {minute}'
                  </span>
                </div>

                <div className="flex items-center gap-4 sm:gap-8">
                  <span className="text-5xl sm:text-7xl font-black text-white italic tracking-tighter drop-shadow-2xl">{currentHomeScore}</span>
                  <span className="text-2xl sm:text-3xl font-black text-zinc-800">:</span>
                  <span className="text-5xl sm:text-7xl font-black text-white italic tracking-tighter drop-shadow-2xl">{currentAwayScore}</span>
                </div>

                <Badge className={cn(
                  "px-3 sm:px-5 py-1 rounded-lg font-black text-[9px] sm:text-xs uppercase tracking-widest",
              ...
              <div className="flex-1 flex flex-col items-center gap-2 sm:gap-4">
                ...
                <div className="text-center">
                  <h2 className="text-sm sm:text-xl font-black text-white tracking-tight uppercase italic leading-tight">{awayClub?.name}</h2>
                  <p className="text-[8px] sm:text-[10px] font-black text-zinc-500 uppercase tracking-[0.2em] mt-0.5">AWAY</p>
                </div>
              </div>
            </div>
          </CardContent>
        ...
        <div className="flex items-center gap-2 border-b border-white/5">
          {(['pitch', 'feed', 'tactics'] as const).map(tab => {
            const labels = { pitch: '2D PITCH', feed: 'MATCH FEED', tactics: 'TACTICAL SETUP' };
            return (
              <button
                key={tab}
                onClick={() => setActiveSubTab(tab)}
                className={cn(
                  "px-4 sm:px-6 py-3 text-[10px] font-black uppercase tracking-[0.15em] transition-all relative whitespace-nowrap",
                  activeSubTab === tab ? "text-indigo-400" : "text-zinc-500 hover:text-zinc-300"
                )}
              >
                {labels[tab]}
                {activeSubTab === tab && <div className="absolute bottom-0 left-0 right-0 h-1 bg-indigo-400 rounded-t-full" />}
              </button>
            );
          })}
        </div>
